...
small_axis <- function(label = NULL, fontsize = 7, arrow_length = 25, fix_coord = FALSE, arrow_offset=6,
  ...
  lines <- annotation_custom(grid::polylineGrob(x = unit(c(arrow_offset, arrow_offset, arrow_length), units), 
                                                y = unit(c(arrow_length, arrow_offset, arrow_offset), units), 
                                                gp = grid::gpar(fill = "black"),
                                                arrow = arrow_spec))
  text <- if(! is.null(label)){
    list(annotation_custom(grid::textGrob(label = paste0(label,"1"), gp = grid::gpar(fontsize = fontsize, fontface="bold"),
                                          x = unit(arrow_offset, units), y = unit(arrow_offset, units), 
                                          hjust = 0, vjust = 1.4)),
         annotation_custom(grid::textGrob(label = paste0(label,"2"), rot=90, gp = grid::gpar(fontsize = fontsize, fontface="bold"),
                                          x = unit(arrow_offset, units), y = unit(arrow_offset, units), 
                                          hjust = 0, vjust = -0.45)))
