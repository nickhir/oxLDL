...
for (i in PC_pairings) {
    ...
    ggsave(
        plot = final_plot,
        filename = here(
            "QC", "results", "imgs", str_interp("PCA_all_covariates_PC${i[1]}_PC${i[2]}.png")
        ),
        dpi = 350,
        width = 20,
        height = 15,
...
why they behave so differently. Maybe remove them.
The outliers have PC1 > 100 and PC2 < -100

```{r}
library(plotly)

# hover over the points to figure out what the outliers are
# plot_ly(data = pca_plot_df, x = ~PC1, y = ~PC2, text = ~sample_id, mode = "markers")
outlier_samples <- pca_plot_df %>%
    filter(PC1 > 100) %>%
    filter(PC2 < -100) %>%
    pull(sample_id)

outlier_lines <- mdata %>% filter(sampleid %in% outlier_samples) %>% pull(line) %>% sort()



## get the cell lines that have technical replicates
technical_replicates_lines <- mdata %>%
    group_by(line) %>%
    mutate(n = n()) %>%
    filter(n > 6) %>%
    pull(line) %>%
...
# unfortunately no overlap
outlier_lines  %in% technical_replicates_lines
...
# just out of interest, also show where the sample with the huge library size falls
...
png(here("QC", "results", "imgs", "SVA_20", "association_heatmap.png"), res=350, width=9, height=5.5, units="in")
correlation_SVs$heatmap
dev.off()

#plot_continuous_covar(mdata, "libsize", "SV1")
