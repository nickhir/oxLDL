...
norm_data <- assay(vsd_data)
mod  <- model.matrix(~ sampletype, colData(dds0))
mod0 <- model.matrix(~ 0 + SV1 + SV2 + SV3 + SV4 + SV5 + SV6 + SV7 + SV8 + SV9 + SV10, colData(dds0))
...
ggsave(
    plot = final_plot,
    filename = here("QC", "results", "imgs", "SVA_10","PCA_all_covariates_PC1_PC2_corrected.png"),
    dpi = 350,
    width = 20,
    height = 15,
)
