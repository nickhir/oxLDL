...
svseq <- svaseq(norm_counts, mod, mod0, n.sv = 15)
saveRDS(svseq, here("QC", "results", "sva_15SV.rds"))
...
plot_categorical_covar(mdata %>% mutate(SV1=SV1*40), covar = "pool", PC="SV1")
plot_categorical_covar(mdata %>% mutate(SV1=SV1*40) , covar = "batch1id", PC="SV1")
plot_categorical_covar(mdata %>% mutate(SV1=SV1*40) %>% mutate(oxldlset=factor(oxldlset)) , covar = "oxldlset", PC="SV1")
plot_categorical_covar(mdata %>% mutate(SV10 = SV10 * 40) %>% mutate(oxldlset = factor(oxldlset)),
    covar = "oxldlset", PC = "SV10"
)

plot_categorical_covar(mdata,
    covar = "set", PC = "SV1"
)


tmp_cov <- mdata[, "oxldlset"]
tmp_component <-mdata[, "SV10"]

get_assoc_pval_categorical(x = tmp_cov, y = tmp_component)
broom::tidy(kruskal.test(tmp_component ~ tmp_cov))$p.value
