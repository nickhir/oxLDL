...
show_batch(
    dds = dds_subset,
    value = "79704",
    sampleids = mdata %>% filter(batch1id == 79704) %>% filter(group == "M0_ctrl")  %>% pull(sampleid),
    output_adjusted = F
)[2]

swaps_79704 <- c(swaps_79704, 
"MM_oxLDL9752216", "MM_oxLDL9752168", # M0_stim outliers
"MM_oxLDL9752218", # M1_stim outliers
"MM_oxLDL9752179" # M0_ctrl outlier
)

# this still better now.
show_batch(
    dds = dds,
    value = "79704",
    sampleids = swaps_79704,
...
mdata_new %>% mutate(group = replace(group, var))
