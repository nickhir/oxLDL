...
calc_pairwise_cor <- function(mat, outliers, condition){
    keep_samples <- mdata %>% filter(sampletype == condition)  %>% pull(sampleid)
...
mdata <- mdata %>%
    mutate(group = paste(state, oxldl, sep="_")) 
...
h <- draw(h)
outliers_M0_0 <- column_order(draw(h))$`1`
outliers_M0_0 <- rownames(cor_result$M0_0)[outliers_M0_0]
...
ggplot(
    pca_plot_df_subset %>% mutate(outlier = ifelse(sample_id %in% outliers_M0_0, "MO_0 outlier", "normal")),
    aes(x = PC1, y = PC2, color = outlier)
) +
    geom_point(size = 3.5) +
    ggrepel::geom_label_repel(data = pca_plot_df_subset %>% filter(sample_id %in% outliers_M0_0), 
...
ggplot(
    pca_plot_df_subset %>% mutate(outlier = ifelse(sample_id %in% outliers_M0_0, "MO_0 outlier", "normal")),
    aes(x = libsize, y = ..scaled.., fill = outlier)
) +
    geom_density(alpha = 0.5) +
    chameleon::scale_fill_chameleon() +
    theme_Publication_side()

# -> the outliers have very small libsize in comparison. Lets remove them -> this is data driven instead of just 
# using a fixed cutoff. 
M0_0_filtered <- calc_pairwise_cor(
    mat = adjusted_counts,
    outliers = outliers_M0_0,
    condition ="M0_0"
)
M0_0_filtered$correlation_matrix %>% min()
```
...
outliers_M2_0 <- apply(cor_result$M2_0, 2, mean) %>% which.min() %>% names()



# recalculate PCA with adjusted counts
pca_data_subset <- PCAtools::pca(adjusted_counts[, rownames(cor_result$M2_0)],
    metadata = colData(dds)[rownames(cor_result$M2_0),],
    removeVar = 0.4
)
pca_plot_df_subset <- pca_data_subset$rotated[, 1:20] %>%
    rownames_to_column("sample_id") %>%
...
ggplot(
    pca_plot_df_subset %>% mutate(outlier = ifelse(sample_id %in% outliers_M2_0, "MO_0 outlier", "normal")),
    aes(x = PC1, y = PC2, color = outlier)
) +
    geom_point(size = 3.5) +
    ggrepel::geom_label_repel(data = pca_plot_df_subset %>% filter(sample_id %in% outliers_M2_0), 
    aes(x = PC1, y = PC2, label = sample_id), 
    inherit.aes = F) +
    theme_Publication()+
    chameleon::scale_color_chameleon()
...
outliers_M2_0_libsize <- mdata %>%
    filter(sampleid == outliers_M2_0) %>%
    pull(libsize)

# libsize is 0.51 percentile --> actually the 6th smalles. 
percentile <- ecdf(mdata$libsize)(outliers_M2_0_libsize) * 100

# -> the outliers have very small libsize in comparison. Lets remove them -> this is data driven instead of just 
# using a fixed cutoff. 
M2_0_filtered <- calc_pairwise_cor(
    mat = adjusted_counts,
    outliers = outliers_M2_0,
    condition ="M2_0"
)
M2_0_filtered$correlation_matrix %>% min()
```
...
# looks like there is really just one sample which is very different
outliers_M0_100 <- apply(cor_result$M0_100, 2, mean) %>% which.min() %>% names()



# recalculate PCA with adjusted counts
pca_data_subset <- PCAtools::pca(adjusted_counts[, rownames(cor_result$M0_100)],
    metadata = colData(dds)[rownames(cor_result$M0_100),],
    removeVar = 0.4
)
pca_plot_df_subset <- pca_data_subset$rotated[, 1:20] %>%
    rownames_to_column("sample_id") %>%
...
ggplot(
    pca_plot_df_subset %>% mutate(outlier = ifelse(sample_id %in% outliers_M0_100, "MO_0 outlier", "normal")),
    aes(x = PC1, y = PC2, color = outlier)
) +
    geom_point(size = 3.5) +
    ggrepel::geom_label_repel(data = pca_plot_df_subset %>% filter(sample_id %in% outliers_M0_100), 
    aes(x = PC1, y = PC2, label = sample_id), 
    inherit.aes = F) +
    theme_Publication()+
    chameleon::scale_color_chameleon()
...
outliers_M0_100_libsize <- mdata %>%
    filter(sampleid == outliers_M0_100) %>%
    pull(libsize)

# libsize is 0.68 percentile --> actually the 8th smalles. 
percentile <- ecdf(mdata$libsize)(outliers_M0_100_libsize) * 100

# -> the outliers have very small libsize in comparison. Lets remove them -> this is data driven instead of just 
# using a fixed cutoff. 
M0_100_filtered <- calc_pairwise_cor(
    mat = adjusted_counts,
    outliers = outliers_M0_100,
    condition ="M0_oxLDL"
)
M0_100_filtered$correlation_matrix %>% min()
```
...
# looks like there is really just one sample which is very different
outliers_M1_100 <- apply(cor_result$M1_100, 2, mean) %>% which.min() %>% names()



# recalculate PCA with adjusted counts
pca_data_subset <- PCAtools::pca(adjusted_counts[, rownames(cor_result$M1_100)],
    metadata = colData(dds)[rownames(cor_result$M1_100),],
    removeVar = 0.4
)
pca_plot_df_subset <- pca_data_subset$rotated[, 1:20] %>%
    rownames_to_column("sample_id") %>%
...
ggplot(
    pca_plot_df_subset %>% mutate(outlier = ifelse(sample_id %in% outliers_M1_100, "MO_0 outlier", "normal")),
    aes(x = PC1, y = PC2, color = outlier)
) +
    geom_point(size = 3.5) +
    ggrepel::geom_label_repel(data = pca_plot_df_subset %>% filter(sample_id %in% outliers_M1_100), 
    aes(x = PC1, y = PC2, label = sample_id), 
    inherit.aes = F) +
    theme_Publication()+
    chameleon::scale_color_chameleon()
...
outliers_M1_100_libsize <- mdata %>%
    filter(sampleid == outliers_M1_100) %>%
    pull(libsize)

# libsize is 0.25 percentile --> actually the 3th smalles. 
percentile <- ecdf(mdata$libsize)(outliers_M1_100_libsize) * 100

# -> the outliers have very small libsize in comparison. Lets remove them -> this is data driven instead of just 
# using a fixed cutoff. 
M1_100_filtered <- calc_pairwise_cor(
    mat = adjusted_counts,
    outliers = outliers_M1_100,
    condition ="M1_oxLDL"
)
M1_100_filtered$correlation_matrix %>% min()
```
...
# recalculate PCA with adjusted counts
pca_data_subset <- PCAtools::pca(adjusted_counts[, rownames(cor_result$M2_100)],
    metadata = colData(dds)[rownames(cor_result$M2_100),],
    removeVar = 0.4
)
pca_plot_df_subset <- pca_data_subset$rotated[, 1:20] %>%
    rownames_to_column("sample_id") %>%
...
ggplot(pca_plot_df_subset, aes(x = PC1, y = PC2, color="set")) +
    geom_point(size = 3.5) +
    theme_Publication() +
    chameleon::scale_color_chameleon()
...
```{r}
outliers <- c(
    outliers_M0_0, outliers_M1_0, outliers_M2_0,
    outliers_M0_100, outliers_M1_100
)

dds <- readRDS(here("QC", "results", "dds_libsize_outliers_removed.rds"))
dds_filtered <- dds[, !dds$sampleid  %in% outliers]
saveRDS(dds_filtered, here("QC", "results", "dds_libsize_outliers_ENCODE_removed.rds"))
