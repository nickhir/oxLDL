...
dummy_ <- parallel::mclapply(5:20, function(i) {
    ...
    png(here("QC", "results", "imgs", str_interp("SVA_${i}"), "association_heatmap.png"), res = 150, width = 9, height = 5.5, units = "in")
...
dummy_ <- parallel::mclapply(5:20, function(i){
    ...
    for (z in 1:2) {
        ...
        sig_results <- enrichment_result@result %>%
            ...
            # write the visulaization dotplot

            ggplot(sig_results, aes(x = ratio_numeric, y = Description)) +
                geom_segment(aes(
                    x = 0, xend = ratio_numeric, y = Description, yend = Description                    
                ), color = "black") +
                geom_point(size = 5) +
