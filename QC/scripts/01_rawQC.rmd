...
based on the read level (for example # Duplicated reads, GC content, mapping rate, read distribution, …). 
...
### Filter genes
Keep protein coding and lnc genes with mean expr > 0.5 TPM per condition in at least half of the conditions (3)

```{r}
# remove some lowly expressed genes
tpm_data <- data.table::fread(here("preprocessing", "results", "gene_counts_tpm.tsv"), data.table = FALSE)
tpm_long <- tpm_data %>%
    pivot_longer(-gene_name, names_to = "sample", values_to = "tpm") %>%
    left_join(., mdata %>% select(sampleid, group), by = c("sample" = "sampleid"))
group_wise_avg <- tpm_long %>%
    group_by(gene_name, group) %>%
    summarise(avg_tpm = mean(tpm))  %>% 
    ungroup()
# get list of genes to keep
keep_genes <- group_wise_avg %>%
    group_by(gene_name) %>%
    summarize(high_tpm_count = sum(avg_tpm > 0.5)) %>% 
    filter(high_tpm_count >= 3) %>%
    pull(gene_name)

# now remove the genes that are not protein coding or lnc
gene_info <- rtracklayer::import(here("data", "oxLDL_data", "RNA_seq", "results", "genome", "Homo_sapiens.GRCh38.dna.primary_assembly_genes.gtf")) %>%
    as.data.frame() %>%
    filter(type == "gene") %>%
    select(gene_id, gene_name, gene_biotype)

keep_genes <- keep_genes[keep_genes %in% (gene_info %>%
    filter(gene_biotype %in% c("protein_coding", "lncRNA")) %>%
    pull(gene_name))]


dds0 <- dds0[rownames(dds0) %in% keep_genes, ]
vsd_data <- vst(dds0)
...
For PC1 there are some big outliers that drive the majority of variation in them. Identify them and try to see 
why they behave so differently. 

Turns out they have time lib size -> remove them
