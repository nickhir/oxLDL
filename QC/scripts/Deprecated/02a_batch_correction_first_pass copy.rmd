...
association_output <- covar_correlation(
    df = pca_plot_df,
    covars = c("pool", "state", "condition", "oxldlset", "sex", "ethnicity", "age", "batch1id"),
    components = paste0("PC", 1:15)
)
...
M2_comp_res_conventional <- results(init_deseq_res, contrast=c("group", "M2_oxLDL", "M2_ctrl"))
...
M0_comp_res_conventional <- results(init_deseq_res, contrast=c("group", "M0_oxLDL", "M0_ctrl"))
...
M2_M1_comp_res_conventional <- results(init_deseq_res, contrast=c("group", "M2_ctrl", "M1_ctrl"))
identical(data.frame(M0_comp_res), data.frame(M0_comp_res_conventional))
...
ggplot(
    pca_plot_df_subset %>% mutate(outlier = ifelse(sample_id %in% outliers_M0_ctrl, "MO_0 outlier", "normal")),
    aes(x = PC1, y = PC2, color = outlier)
) +
    geom_point(size = 3.5) +
    ggrepel::geom_label_repel(data = pca_plot_df_subset %>% filter(sample_id %in% outliers_M0_ctrl), 
...
ggplot(
    pca_plot_df_subset %>% mutate(outlier = ifelse(sample_id %in% outliers_M1_ctrl, "MO_0 outlier", "normal")),
    aes(x = PC1, y = PC2, color = outlier)
) +
    geom_point(size = 3.5) +
    ggrepel::geom_label_repel(data = pca_plot_df_subset %>% filter(sample_id %in% outliers_M1_ctrl), 
    aes(x = PC1, y = PC2, label = sample_id), 
    inherit.aes = F) +
    theme_Publication()+
    chameleon::scale_color_chameleon()
...
outliers_M1_ctrl_libsize <- mdata %>%
    filter(sampleid == outliers_M1_ctrl) %>%
    pull(libsize)

# libsize is 0.42 percentile
percentile <- ecdf(mdata$libsize)(outliers_M1_ctrl_libsize) * 100

# -> the outliers have very small libsize in comparison. Lets remove them -> this is data driven instead of just 
# using a fixed cutoff. 
M1_0_filtered <- calc_pairwise_cor(
    mat = adjusted_counts,
    outliers = outliers_M1_ctrl,
    condition ="M1_ctrl"
)
M1_0_filtered$correlation_matrix %>% min()
```
...
ggplot(
    pca_plot_df_subset %>% mutate(outlier = ifelse(sample_id %in% outliers_M2_ctrl, "MO_0 outlier", "normal")),
    aes(x = PC1, y = PC2, color = outlier)
) +
    geom_point(size = 3.5) +
    ggrepel::geom_label_repel(data = pca_plot_df_subset %>% filter(sample_id %in% outliers_M2_ctrl), 
    aes(x = PC1, y = PC2, label = sample_id), 
    inherit.aes = F) +
    theme_Publication()+
    chameleon::scale_color_chameleon()
...
outliers_M2_ctrl_libsize <- mdata %>%
    filter(sampleid == outliers_M2_ctrl) %>%
    pull(libsize)

# libsize is 0.51 percentile --> actually the 6th smalles. 
percentile <- ecdf(mdata$libsize)(outliers_M2_ctrl_libsize) * 100

# -> the outliers have very small libsize in comparison. Lets remove them -> this is data driven instead of just 
# using a fixed cutoff. 
M2_0_filtered <- calc_pairwise_cor(
    mat = adjusted_counts,
    outliers = outliers_M2_ctrl,
    condition ="M2_ctrl"
)
...
ggplot(
    pca_plot_df_subset %>% mutate(outlier = ifelse(sample_id %in% outliers_M0_stimm, "MO_0 outlier", "normal")),
    aes(x = PC1, y = PC2, color = outlier)
) +
    geom_point(size = 3.5) +
    ggrepel::geom_label_repel(data = pca_plot_df_subset %>% filter(sample_id %in% outliers_M0_stimm), 
    aes(x = PC1, y = PC2, label = sample_id), 
    inherit.aes = F) +
    theme_Publication()+
    chameleon::scale_color_chameleon()
...
outliers_M0_stimm_libsize <- mdata %>%
    filter(sampleid == outliers_M0_stimm) %>%
    pull(libsize)

# libsize is 0.68 percentile --> actually the 8th smalles. 
percentile <- ecdf(mdata$libsize)(outliers_M0_stimm_libsize) * 100

# -> the outliers have very small libsize in comparison. Lets remove them -> this is data driven instead of just 
# using a fixed cutoff. 
M0_100_filtered <- calc_pairwise_cor(
    mat = adjusted_counts,
    outliers = outliers_M0_stimm,
    condition = "M0_stim"
)
...
ggplot(
    pca_plot_df_subset %>% mutate(outlier = ifelse(sample_id %in% outliers_M1_stimm, "MO_0 outlier", "normal")),
    aes(x = PC1, y = PC2, color = outlier)
) +
    geom_point(size = 3.5) +
    ggrepel::geom_label_repel(data = pca_plot_df_subset %>% filter(sample_id %in% outliers_M1_stimm), 
    aes(x = PC1, y = PC2, label = sample_id), 
    inherit.aes = F) +
    theme_Publication()+
    chameleon::scale_color_chameleon()
...
outliers_M1_stimm_libsize <- mdata %>%
    filter(sampleid == outliers_M1_stimm) %>%
    pull(libsize)

# libsize is 0.25 percentile --> actually the 3th smalles. 
percentile <- ecdf(mdata$libsize)(outliers_M1_stimm_libsize) * 100

# -> the outliers have very small libsize in comparison. Lets remove them -> this is data driven instead of just 
# using a fixed cutoff. 
M1_100_filtered <- calc_pairwise_cor(
    mat = adjusted_counts,
    outliers = outliers_M1_stimm,
    condition ="M1_stim"
)
...
ggplot(pca_plot_df_subset, aes(x = PC1, y = PC2, color="sex")) +
    geom_point(size = 3.5) +
    theme_Publication() +
    chameleon::scale_color_chameleon()
...
```{r}
outliers <- c(
    outliers_M0_ctrl, outliers_M1_ctrl, outliers_M2_ctrl,
    outliers_M0_stimm, outliers_M1_stimm
)
