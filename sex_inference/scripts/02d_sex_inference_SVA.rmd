...
On our data, it performes "worse", i.e. more missmatches. 
```{r, eval=FALSE}
# Split the data into training and test set
set.seed(50)
training_samples <- mdata$sex %>% 
...
waterfall_plot <- ggplot(data, aes(x = reorder(gene, Coefficient), y = Coefficient, fill = reorder(gene, Coefficient), label = gene)) +
    geom_col(show.legend = TRUE) +
    geom_text(aes(y = Coefficient),
        position = position_dodge(width = 0.9),
        vjust = ifelse(data$Coefficient >= 0, -0.5, 1.5),
        fontface = "bold"
    ) +
    scale_fill_manual(values = c(
        "#BD1616", "#82E0AA", "#58D68D",
        "#2ECC71", "#28B463", "#239B56"
    )) +
    labs(
...
ggsave(
    plot = waterfall_plot,
    filename = here("sex_inference", "results", "waterfallplot_featureImportance.png"),
    width = 7,
    height = 5,
    dpi = 350,
    scale = 1.35
...
distribution_plots <- lapply(predictive_genes[-which(predictive_genes == "(Intercept)")], function(gene) {
    train_data <- scale(adjusted_counts[gene, ])
    our_data <- scale(adjusted_counts_our_data[gene, ])
    min_value <- floor(min(train_data, our_data))
    max_value <- as.integer(max(train_data, our_data)) + 1
    train_plot <- plot_density(train_data, mdata$sex) +
...
### Investigate the missmatches
We do not observe any enrichment of missmatches in any condition/group
```{r}
# Function to generate a density plot
plot_density <- function(data_vector, sex = waiver()) {
    # Create the ggplot density plot
...
plot_data_ours<- t(adjusted_counts_our_data)[, predictive_genes[-which(predictive_genes == "(Intercept)")]] %>%
    data.frame() %>%
    rownames_to_column("sampleid") %>%
    pivot_longer(., cols = !sampleid, values_to = "norm_cnts", names_to = "gene") %>%
...
p <- ggplot(plot_data_ours, aes(x = sex, y = norm_cnts)) +
    ggbeeswarm::geom_quasirandom(width = 0.15, size = 0.1) +
